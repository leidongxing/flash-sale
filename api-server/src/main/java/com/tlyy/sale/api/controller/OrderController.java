...
import com.tlyy.sale.api.service.order.OrderV2Service;
import com.tlyy.sale.api.service.order.PreService;
import com.tlyy.sale.api.vo.CreateOrderV3VO;
import com.tlyy.sale.api.vo.CreateOrderVO;
...
//        preService.checkUser(vo.getUid());
    ...
    /**
     * 创建订单 V8  多个队列扣减库存+condition处理+异步延迟处理非核心SQL
     **/
    @PostMapping("/order/v8")
    public CommonResponse createOrderV8(@Validated @RequestBody CreateOrderV3VO vo) {
//        preService.rateLimit();
//        preService.validateToke(vo.getUid(), vo.getItemId(), vo.getToken());
//        preService.checkUser(vo.getUid());
